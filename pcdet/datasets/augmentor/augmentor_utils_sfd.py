...
def random_flip_along_x(gt_boxes, points, points_pseudo):
    """
    Args:
    ...
    if enable:
        ...
        points[:, 1] = -points[:, 1]
        points_pseudo[:, 1] = -points_pseudo[:, 1]

        if gt_boxes.shape[1] > 7:
            gt_boxes[:, 8] = -gt_boxes[:, 8]

    return gt_boxes, points, points_pseudo

def random_flip_along_y(gt_boxes, points, points_pseudo):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
    ...
    if enable:
        ...
        points[:, 0] = -points[:, 0]
        points_pseudo[:, 0] = -points_pseudo[:, 0]
...
def global_rotation(gt_boxes, points, points_pseudo, rot_range):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    points_pseudo = common_utils.rotate_points_along_z(points_pseudo[np.newaxis, :, :], np.array([noise_rotation]))[0]
...
def global_scaling(gt_boxes, points, points_pseudo, scale_range):
    """
    Args:
...
def noise_per_object_v3_(gt_boxes,
    ...
    if (points is not None) and (points_pseudo is not None):
